...
import { runLogin, type ProviderName } from './login.js';
...
function parseLoginProvider(arg: string | undefined): ProviderName | null {
  if (!arg) {
    return null;
  }

  const provider = arg.trim().toLowerCase();
  const validProviders: ProviderName[] = ['claude', 'gemini', 'zai', 'codex'];
