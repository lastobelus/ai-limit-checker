...
function formatDebounceDuration(ms: number): string {
  if (ms <= 0) {
    return 'off';
  }
...
function formatClaudeDebounceHeader(results: LlmLimitStatus[]): string | null {
...
export function formatPrettyOutput(results: LlmLimitStatus[]): string {
  const lines: string[] = [];
